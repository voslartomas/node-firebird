...
function fetch_blob_sync(rows, meta, isSelect, callback) {
    ...
    doSynchronousLoop(rows, function(row, i, next) {
        doSynchronousLoop(blob_col, function(col, j, next2) {
            if (!row[col])
                return next2();
            row[col](function (err, name, e) {
                if (err)
                    return next2(err)
                ...
                e.on('end', function (chunck) {
                    row[col] = Buffer.concat(_d, _dlength);
                    next2()
                });
...
Database.prototype.attachEvent = function (callback) {
    ...
    this.connection.connectRequest(function(err, socket_info) {
        var cnxevent = new EventConnection(socket_info.host, socket_info.port, function(err) {
...
FbEventManager.prototype.registerEvent = function(events, callback) {
    ...
    if (self.db.connection._isClosed || self.eventconnection._isClosed)
        return self.throwClosed(callback);
                ...
                switch (r) {
                    case op_event:
                        ...
                        while (pos < buf.length) {
                            ...
                            var prevcount = self.emgr.events[eventname];
                            pos += len;
                            eventcount = buf.readInt32LE(pos);
                            self.emgr.events[eventname] = eventcount
                            pos += 4;
        ...
        switch (r) {
            ...
            case op_accept:
                if (data.readInt() !== PROTOCOL_VERSION10 || data.readInt() !== ARCHITECTURE_GENERIC || data.readInt() !== ptype_batch_send)
                    return callback(new Error('Invalid connect result'));
                return cb(null, {});
            ...
            case op_event:
                ...
                while (pos < buf.length) {
                    var len = buf.readInt8(pos++);
                    eventname = buf.toString(DEFAULT_ENCODING, pos, pos+len);
                    var prevcount = db.events[eventname];                   
                    pos += len;
                    eventcount = buf.readInt32LE(pos);
                    db.events[eventname] = eventcount
                    pos += 4;
                    if (prevcount !== 0 && prevcount !== eventcount)               
                        lst_event.push({ name: eventname, count: eventcount });  
                }
                data.readInt64(); // ignore AST INFO
                var event_id = data.readInt();
                return cb(null, {eventid : event_id, events : lst_event});    
            default:
                return cb(new Error('Unexpected:' + r));
        }
    } catch (err) {
...
Connection.prototype.connectRequest = function (callback) {
    ...
    function cb(err, ret) {
        ...
        var socket_info = {
            ...
            host : ret.buffer.readInt8(4) + '.' + ret.buffer.readInt8(5) + '.' + ret.buffer.readInt8(6) + '.' + ret.buffer.readInt8(7)
        }
